...
/**
 * A generic Functor trait that defines the mapping operation
 * for any type constructor F[_]
 */
trait Functor[F[_]]:
 ...
 * 
 * Since EnergyData is not a type constructor but a concrete type,
 * we use a type alias to create a type constructor that wraps Double 
 * (the value in EnergyData)
 */
...
/**
 * Usage examples showing how to apply the functor concept 
 * to the REPS codebase
 */
object FunctorExamples:
  ...
  def scaleAllReadings(readings: List[EnergyData], scaleFactor: Double): List[EnergyData] =
    transformEnergyData(
      readings,
      summon[Functor[EnergyFunctor.EnergyDataT]],
      _ * scaleFactor
  ...
  def normalizeAndConvertTokW(readings: List[EnergyData], maxCapacity: Double): List[EnergyData] =
  ...
  def main(args: Array[String]): Unit =
    ...
    val solarReading = EnergyData(EnergyType.Solar, now, now.plusHours(1), 150.0)
    val windReading = EnergyData(EnergyType.Wind, now, now.plusHours(1), 220.0)
    val hydroReading = EnergyData(EnergyType.Hydro, now, now.plusHours(1), 350.0)
    ...
    normalizedReadings.foreach(data => 
      println(s"${data.energyType}: ${data.value} kW (${data.value/1000 * 100}%)")
    )